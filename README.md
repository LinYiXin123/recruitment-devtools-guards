...
![招聘平台 DevTools 守卫双平台毛玻璃弹窗](./docs/images/dual-platform-glass-ui.png)

上图为双平台集成版 `1.1.0`：BOSS 直聘与猎聘分别提供独立开关、当前页面状态提示和站点快捷链接。
...
![未启用扩展时在猎聘网页打开 F12 开发者工具的表现](./docs/images/liepin-normal-f12-devtools.webp)

上图记录未启用本扩展时，在猎聘网页打开 F12 开发者工具后跳转到 `about:blank` 的页面表现。

### 启用扩展

![启用扩展后在猎聘网页正常打开 F12 开发者工具](./docs/images/liepin-extension-enabled-f12.webp)

上图记录启用本扩展后，猎聘网页可在 F12 开发者工具保持打开时正常浏览，已确认的反调试跳转链不再触发。
